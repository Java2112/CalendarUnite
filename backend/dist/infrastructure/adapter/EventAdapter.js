...
class EventAdapter {
    ...
    async findAll(modality, search) {
        ...
        if (modality && modality !== 'Todas') {
            filtered = filtered.filter(e => (e.modalidad || e.modality || '').toLowerCase() === modality.toString().toLowerCase());
        }
        if (search) {
            const q = search.toString().toLowerCase();
            filtered = filtered.filter(e => (e.nombre || e.title || '').toLowerCase().includes(q) ||
    ...
    async update(id, data, bannerUrl) {
        ...
        if (data.nombre)
            updated.title = data.nombre;
        if (data.modalidad)
            updated.modality = data.modalidad;
        if (data.fecha_inicio)
            updated.date = data.fecha_inicio;
        if (data.hora_inicio && data.hora_fin)
            updated.time = `${data.hora_inicio} - ${data.hora_fin}`;
        if (bannerUrl) {
    ...
    async findAttendeeByEventAndEmail(eventId, email) {
        const attendee = this.attendees.find(a => (a.eventId === eventId || a.eventId === `evt-${eventId}`) && a.correo.toLowerCase() === email.toLowerCase());
